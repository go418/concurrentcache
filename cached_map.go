...
// If the cached value is not older than the provided minVersion, the cached value is returned.
...
func (c *CachedMap[K, V]) Get(ctx context.Context, key K, minVersion CacheVersion) Result[V] {
	...
	case <-ctx.Done(): // The context was canceled before the worker finished.
		...
		return Result[V]{
			Value:       worker.returnValue.value,
			Error:       errors.Join(worker.returnValue.err, context.Cause(ctx)),
			FromCache:   false,
	...
	return worker.returnValue.toResult(false)
}
...
// The provided version must not be older than the current version of the cached value otherwise
...
func (c *CachedMap[K, V]) Set(key K, value V, minVersion CacheVersion) {
	...
	c.setLocked(key, value, minVersion)
}
...
func (c *CachedMap[K, V]) setLocked(key K, value V, minVersion CacheVersion) {
	item := c.items[key]

	// If the version is NonCachedVersion, panic.
	if minVersion == NonCachedVersion {
	...
	// as any results that will be generated by the Generate function. Such that calling Get(..., result.NextVersion)
	// will return a version that is newer than the version we are setting here.
	if minVersion == AnyVersion {
		if item.worker != nil {
			minVersion = item.worker.returnValue.sameAge()
		} else {
			minVersion = versionsinternal.NewGlobalVersion()
		}
	}

	// If the cached value is already at least as new as the new version, do nothing.
	if item.cachedValue.isNotOlderThan(minVersion) {
		return
	}

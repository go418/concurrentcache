...
// Get returns the result generated by the Generate function.
...
func (c *CachedItem[V]) run(ctx context.Context, worker *cacheWorker[V]) {
	...
	if worker.nrGetCallsWaiting > 0 {
		c.cachedValue = worker.returnValue
	}
}
...
func (c *CachedItem[V]) Set(ctx context.Context, value V, minVersion CacheVersion) {
	...
	if minVersion == AnyVersion {
		if c.worker != nil {
			minVersion = c.worker.returnValue.sameAge()
		} else {
	...
	if c.cachedValue.isNotOlderThan(minVersion) {
		return
	}
